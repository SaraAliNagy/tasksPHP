...
class Subcategory extends config implements operations{
    ...
    {
        $quary =  "SELECT `id`,`name_en` FROM `subcategories` WHERE `categories_id` = $this->categories_id AND status = $this->status";
        return $this->runDQL($quary);

    }
    public function sherchId()
    {
        $quary =  "SELECT `id`,`name_en` FROM `subcategories` WHERE `id` = $this->id AND status = $this->status";
        return $this->runDQL($quary);
